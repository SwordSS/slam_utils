...
void ScanOdom::Run()
{
    while(ros::ok())
    {
        ...
        m_slice_queue.GetScanSliceBag(one_ScanSlice_bag);
        sensor_msgs::LaserScanConstPtr scan_msg = one_ScanSlice_bag.vector_ptr_scan[0];
        ...
        if(scan_odom_status==Initialzing)
        {
            ...
            bool b_tf_flag = GetLaserInBase(scan_msg,m_T_laserG_in_base);//这些行为后续可能也会出现判断情况,先由外面进行一定的变换矫正，实现相对麻烦
